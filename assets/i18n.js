...
// it. The lines each string has to hold are reserved in styles/main.css, and the copy is
// written to sit inside them rather than padded out to fill them.
(function () {
  var DICT = {
    ...
    'The block start, the transition and the top-end mechanics, taken one at a time. Short reps with long rests, so every rep is run at the speed you are actually training for.':
      '블록 스타트, 전환 구간, 최고 속도 메커닉을 하나씩 나눠 익혀요. 짧게 반복하고 길게 쉬면서, 매 반복을 실제 목표 속도 그대로 달려요.',
    'The half metre that decides the shot. Keep the ball low through the setup, push off the front foot to create the gap, then land square and rise — no drift, no fade. Drilled slowly first, then at the speed a defender actually closes.':
      '슛을 가르는 반 걸음이에요. 셋업 내내 공을 낮게 두고, 앞발로 밀어 간격을 만든 다음, 정면으로 착지해 그대로 올라와요. 밀리지도, 젖혀지지도 않게. 천천히 익힌 뒤 수비가 실제로 붙는 속도로 올려요.',
    ...
    'Agility fails where strength runs out. Single-leg work and controlled decelerations build the base that lets you change direction late without losing the line.':
      '민첩성은 근력이 떨어지는 지점에서 무너져요. 한 다리 운동과 감속 제어로 기초를 쌓아, 늦게 방향을 틀어도 라인이 흐트러지지 않게 해요.',
    'Quick feet that survive contact with a real game. Ladder patterns first, then the same patterns with a ball and a defender in front of you.':
      '실제 경기에서도 버티는 빠른 발을 만들어요. 래더 패턴을 먼저 익히고, 같은 패턴을 공과 수비를 두고 다시 반복해요.',
    ...
    'A short set that finds the point where your legs stop cooperating, then keeps the form together past it. Built to be repeated at the end of any session.':
      '다리가 말을 듣지 않기 시작하는 지점을 찾고, 그 지점을 지나서도 자세를 지키는 짧은 세트예요. 어떤 세션 끝에 붙여도 반복할 수 있어요.',
    'Working against the other stance without reaching. Angle out of the lead hand, keep the rear foot loaded, and land the counter on the way past.':
      '반대 스탠스를 상대할 때 팔부터 뻗지 않아요. 앞손으로 각을 만들고 뒷발에 체중을 남긴 채, 스쳐 지나가며 카운터를 넣어요.',
    'Depth before power. Recover to the middle every ball, take the high one early, and hold the rally until the short one you can actually attack shows up.':
      '파워보다 깊이가 먼저예요. 매 공마다 가운데로 돌아오고, 높은 공은 일찍 처리하고, 공격할 수 있는 짧은 공이 올 때까지 랠리를 이어가요.',
    'The steps between the highlights. Closeouts, drop steps and the slide that keeps your hips in front, drilled at the speed the game is actually played at.':
      '하이라이트와 하이라이트 사이를 채우는 스텝이에요. 클로즈아웃, 드롭 스텝, 골반을 앞에 두는 슬라이드를 실제 경기 속도로 반복해요.',
    'Some Experience': '경험 조금', 'Advanced': '숙련', 'Anyone': '누구나',
    'Indoor Court': '실내 코트', 'Outdoor Court': '야외 코트', 'Track': '트랙',
    'Gym': '체육관', 'Anywhere': '어디서든',
    'PRIME': '준비', 'DRIVE': '가속', 'SPRINT!': '전력!', 'SET-UP': '세트업',
    'BLOCKS': '블록', 'RACE!': '출발!', 'WARM': '예열', 'DRILL': '드릴',
    'SHOOT!': '슛!', 'MOBILISE': '가동', 'LOAD': '부하', 'HOLD!': '버텨!',
    'LOOSEN': '이완', 'PATTERN': '패턴', 'PLAY!': '실전!', 'SHADOW': '섀도',
    'GHOST!': '고스트!', 'PUSH': '푸시', 'GO!': '고!', 'EASE IN': '예열',
    ...
    'Not now': '나중에', 'Yes': '네',
    /* "want to move?" is shared with the level screen's "How do you want to move?" —
       one entry, and both headings read right off it. */
    'When do you': '언제',
    "We'll send a reminder when it comes round.": '시간이 되면 알림을 보내드려요.',
    'Schedule': '일정', 'Date': '날짜', 'Reminder': '알림', 'Set': '설정', 'Booked': '예약 완료',
    'Today': '오늘', 'Tomorrow': '내일', 'am': '오전', 'pm': '오후',
    '10 minutes before': '10분 전', 'Just enough time to change': '옷 갈아입을 정도의 여유',
    '1 hour before': '1시간 전', 'Time to plan the rest of the day around it': '하루 일정을 맞출 수 있는 여유',
    'At the time': '시작할 때', 'Only when it starts': '시작하는 순간에만',
    /* The confirmation's default line. Once Set is pressed the screen rewrites it from
       whichever reminder was chosen — in the reader's language, in schedule.html. */
    ...
    'See how Newton moves, from setup to report.': '세팅부터 리포트까지, Newton이 움직이는 방식을 볼 수 있어요.',
    'Language': '언어', 'Flow': '경로',
    'First Setup': '첫 세팅',
    'Fit Newton and get ready to move.': 'Newton을 나에게 맞추고 움직일 준비를 해요.',
    'Explore Home': '홈 둘러보기',
    'Find movements, packs, and recent activity.': '움직임과 팩, 최근 기록을 한자리에서 찾아봐요.',
    'Start a Workout': '운동 시작하기',
    'Set the session and start moving.': '오늘 세션을 설정하고 바로 움직여요.',
    'Check Your Report': '리포트 확인하기',
    'See how the movement turned out.': '오늘 움직임이 어땠는지 확인해요.',
    ...
    'You can pick more than one.': '여러 개 골라도 괜찮아요.',
    'What do you': '무엇을', 'want to do most?': '하고 싶나요?',
    'We’ll set the best default mode for you.': '가장 잘 맞는 기본 모드로 맞춰드려요.',
    'Where do you': '주로 어디서', 'usually work out?': '운동하나요?',
    'Indoors': '집 안', 'Mostly indoors': '주로 실내에서 해요',
    'Outdoors': '집 밖', 'Parks, tracks, and': '공원, 트랙 같은', 'another spots': '야외에서요',
    'Both': '둘 다', 'Depends on the day': '그날그날 달라요',
    'Follow cool moves': '멋진 동작 따라 하기',
    'Recovery, Care': '회복 / 케어',
    'Build Sport Skills': '종목 실력 키우기',
    'Play with Friends': '친구와 함께하기',
    ...
    'Hold still — moving to the next step': '그대로 계세요 — 다음 단계로 넘어가요',
    'Now, you’re': '이제', 'ready to move!': '움직일 준비가 됐어요!',
    'We’ve prepared Packs tailored to your choices': '선택한 정보에 맞춰 팩을 준비했어요',
    ...
    'Near-Original': '원본 근접', 'Performing close to the real movement': '실제 움직임에 가까운 수행',
    'How are you': '오늘 컨디션은', 'feeling today?': '어때요?',
    'Condition': '컨디션',
    'Feeling light': '가벼워요', 'My body feels loose and ready': '몸이 잘 풀려 있어요',
    'About the Same as Usual': '평소와 비슷해요', 'Neither great nor bad': '좋지도 나쁘지도 않아요',
    'About the same as usual': '평소와 비슷해요',
    'Feeling Heavy': '몸이 무거워요', 'A little tired or stiff': '조금 피곤하고 뻐근해요',
    'Very Tired': '많이 피곤해요', 'I only want to move lightly': '가볍게만 움직이고 싶어요',
    'Any areas': '불편한 곳이', 'bothering you?': '있나요?',
    'We’ll reduce strain on those areas.': '그 부위에 가는 부담을 덜어드릴게요.',
    ...
    'Quiet On': 'Quiet On', 'Reduced landing impact · minimal indoor noise': '착지 충격을 줄이고 실내 소음을 최소로',
    'Pace On': 'Pace On', 'Auto-corrects knee and ankle wobble': '무릎과 발목의 흔들림을 자동으로 보정',
    'Load On': 'Load On', 'Build strength with added resistance': '저항을 더해 근력을 함께 끌어올려요',
    'Boost On': 'Boost On', 'Automatic pace support in the final stretch': '후반부에 페이스를 자동으로 받쳐줘요',
    'Set your': '실전 운동을', 'main workout': '설정해요',
    'Main Workout': '실전 설정',
    'The 18m Pack (Stretching + Learning) stays as-is': '팩 18분(스트레칭 + 익히기)은 그대로 진행돼요',
    'Distance': '거리', 'Time': '시간', 'For you': '추천',
    'You’re ready': '설정이', 'to move!': '끝났어요!',
    'Your setup is complete.': '이제 움직이기만 하면 돼요.',
    ...
    'No packs match these filters.': '조건에 맞는 팩이 없어요.',
    'Pack Type': '팩 유형', 'Pack Source': '팩 출처',
    'Explore This Pack': '이 팩 살펴보기', 'Joined': '참여', 'Process': '루틴',
    ...
    'Then tap the red pill to pick a flow.': '그다음 빨간 버튼을 누르면 경로를 고를 수 있어요.',

    'Share': '공유', 'Copy link': '링크 복사', 'Link copied': '링크를 복사했어요', 'Close': '닫기'
  };

  // What cannot be listed: the strings that carry a number, and the ones a screen builds
  // at runtime out of parts it chose itself. First match wins. A replacement may be a
  // function when the swap needs a lookup a regex cannot do — the weekday on a date chip.
  var DAY = { Sun: '일', Mon: '월', Tue: '화', Wed: '수', Thu: '목', Fri: '금', Sat: '토' };
  var RULES = [
    [/^(\d+) ?min$/, '$1분'],
    [/^(\d+(?:\.\d+)?)m$/, '$1분'],
    [/^(\d+(?:\.\d+)?)h$/, '$1시간'],
    ...
    // so each part is looked up on its own and the line is rebuilt in order.
    [/^[^·]+(?: · [^·]+)+$/, function (s) {
      return s.split(' · ').map(function (part) { return look(part) || part; }).join(' · ');
    }],
    [/^\+ (\d+)m$/, '+ $1분'],
    // Schedule's own chips and label: "Thu 8/13", "Today · 6:00 pm".
    [/^(Sun|Mon|Tue|Wed|Thu|Fri|Sat) (\d+)\/(\d+)$/, function (s, d, m, day) { return m + '/' + day + ' (' + DAY[d] + ')'; }],
    [/^(\d+):(\d+) (am|pm)$/, function (s, h, m, ap) { return (ap === 'pm' ? '오후 ' : '오전 ') + h + ':' + m; }]
  ];
  function look(key) {
    if (DICT[key]) return DICT[key];
    for (var i = 0; i < RULES.length; i++) {
      if (RULES[i][0].test(key)) {
        var out = key.replace(RULES[i][0], RULES[i][1]);
  ...
  // Attributes worth swapping — the ones a person reads, by eye or by screen reader.
  ...
  window.__lang = lang;
  // For the two screens that BUILD a sentence rather than print one: schedule's reminder
  // line, search's meta row. They ask for the words they are about to join, so what lands
  // in the DOM is already in the right language and the observer has nothing to correct.
  window.__t = function (en) { return (lang() === 'ko' && look(en)) || en; };
